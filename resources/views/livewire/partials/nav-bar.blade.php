<nav class="bg-white border-gray-200 dark:bg-gray-900">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <a href="/" class="flex items-center space-x-3 rtl:space-x-reverse">
            <img src="{{ asset('images/SmallLogo.svg') }}" alt="دار الحديث" class="h-8">
        </a>
    </div>
</nav>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    navbarLinks.forEach(link => {
        link.addEventListener('click', function(event) {
            ...
            if (targetElement) {
                const carouselContainer = document.getElementById('custom-controls-gallery');
                const carouselOffset = carouselContainer.offsetTop;

                window.scrollTo({
                    top: carouselOffset - 100, // Adjust offset as needed
                    behavior: 'smooth'
                });
            }
        });
